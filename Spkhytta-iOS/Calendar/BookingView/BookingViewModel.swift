...
class BookingViewModel: ObservableObject {
    ...
    @Published var userMobile: String = "+46 70 123 45 67"  // Hårdkodat telefonnummer
    ...
    // Ladda användardata från Firebase Auth och SwiftData
    private func loadUserInfo() {
        if let currentUser = Auth.auth().currentUser {
            // Försök hämta från SwiftData först
            let userStorage = UserStorage()
            if let user = userStorage.getUser(by: currentUser.uid) {
                self.userName = user.displayName
                self.userEmail = user.email
            } else {
                // Fallback till Firebase Auth data
                self.userEmail = currentUser.email ?? ""
                self.userName = currentUser.email?.split(separator: "@").first.map(String.init) ?? "Användare"
            }
        } else {
            // Default värden om ingen användare är inloggad
